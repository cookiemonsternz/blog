...
There is a **massive** selection of lighting consoles in the modern day, ranging from simple dmx controllers like the *Chauvet Obey 40* to beasts like the *GrandMA* line of consoles, or *ETC's Hog* series.

I've been lucky enough to be able to play around with a couple MA and Hog consoles, and while they are all uniquely distinct in their workflow and design, they all have several shared flaws which I aim to solve:
...
2. The software - 
Fortunately, most consoles software have free versions which can run on a computer, but out of all softwares I've tried, they all have the same few problems for me, especially in the very physically based representation of fixtures.

3. More on the software -
I'm not sure how much you've seen lighting software, but holy crap it is ugly.

4. Even more on the software -
It is incredible complicated. Software is not very abstracted, for example, in MA3, you have fixtures, which are software representations of the physical fixtures (having properties which directly map to the physical fixture), and then you have 'groups', which are a collection of fixtures, and you apply effects to groups in order to have flip flop effects (think strobe, color chase, position chase, etc). This is not very intuitive, in order to get the functionality I want, its often more complex than the very focussed approach of, you click group, click effect. What if I want to bind the effect to a fader? Use the effect as a crossfade? etc. etc.

5. The hardware -
As I'm sure many of you are aware, lighting consoles typically consist of a number of faders, buttons, and a few rotary encoders. I typically find that faders are good for controlling intensity and speed, and buttons are good for triggering effects. Encoders are typically only used when programming, and are not very useful in a live setting. I think what would be cool is to have console which has more of the 'cool' input methods. Maybe a touchscreen or touchpad, alongside a number of faders. I think by having more analog inputs, we could have a more intuitive and fun console.

# My Design

My design aims to solve (hopefully) all of the above problems, and incorporates custom hardware and software, to run on a Raspberry Pi (or pc). 
...
In the next post, I'll start going over the general design of the console.

### Articulating
What is my problem, and how do I want to solve it?
- My problem is that I want lighting to be more of a performance than having to do predefined scenes, and having to interact with the software a lot while performing. I want it to be more of just moving faders and pressing buttons, and having the software do the rest.
- To solve this, I make a system where inputs are everything. Most outputs are directly connected to inputs and the software isn't really involved in controlling the actual outputs, but rather just in the mapping of inputs to outputs. This keeps the versatility of lighting in general, but allows for a more abstracted way of performing.  
